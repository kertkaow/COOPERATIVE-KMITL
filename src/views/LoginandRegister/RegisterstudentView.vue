<template>
  ...
  <div class="d-flex justify-content-center form">
        <div class="col-md-6 bgbd ">
            <!-- Content goes here -->
            <h1 class="d-flex justify-content-left mb-3 ">Register Student</h1>
...
export default {
   ...
   methods:{
       register(){
          ...
          if(this.email.includes(this.studentID) == false){
             this.errMsg = "รหัสนักศึกษากับอีเมลล์ไม่ตรงกัน"
           }
          else if(this.password !== this.confirmPassword){
             this.errMsg = "รหัสผ่านไม่ตรงกัน"
           }
          else if(containsNumber(this.email) == false){
             this.errMsg = "อีเมลล์ไม่ถูกต้อง"
          }
           else if(this.email.includes("@kmitl.ac.th") == false){
              this.errMsg = "ไม่สามารถลงทะเบียนได้"
