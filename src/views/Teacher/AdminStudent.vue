          ...
          <tr v-for="student in Students" :key="student.id">
            <td>
              <router-link :to="{ path: `/StudentDetails/${student.id}` }"
                class="text-decoration-none btn btn-primary Info-btn" style="color:white;">
                ข้อมูลนักศึกษา
              </router-link>
            </td>
            ...
            <td>
              <router-link class="text-decoration-none" style="color: black;"
                :to="{ path: `/CompanyDetails/${student.firstChoice.project_Id}` }">
                {{ student.firstChoice.companyNameAndProjectName }}</router-link>
              <select v-if="student.gotCompany !== true" v-model="student.firstChoice.result"
                @change="onChangeFirstChoice($event,student.id)" class="selectData">
                <option :value="true">ผ่าน</option>
                <option :value="false">ไม่ผ่าน</option>
                <option :value="''">รอผลสัมภาษณ์</option>
              </select>
              <select v-else-if="student.gotCompany == true" v-model="student.firstChoice.result"
                @change="onChangeFirstChoice($event,student.id)" class="selectData" disabled>
                <option :value="true">ผ่าน</option>
                <option :value="false">ไม่ผ่าน</option>
                <option :value="''">รอผลสัมภาษณ์</option>
              </select>
              <button v-if="student.firstChoice.result === 'true' && student.gotCompany !== true"
                class="btn btn-primary mt-2"
                @click="chooseCompany(student.firstChoice.project_Id,student.firstChoice.companyNameAndProjectName,student.id,student.userId)">เลือกบริษัท</button>
            </td>
            <td>
              <router-link class="text-decoration-none" style="color: black;"
                :to="{ path: `/CompanyDetails/${student.secondChoice.project_Id}` }">
                {{ student.secondChoice.companyNameAndProjectName }}</router-link>
              <select v-if="student.gotCompany !== true" v-model="student.secondChoice.result"
                @change="onChangeSecondChoice($event,student.id)" class="selectData">
                <option :value="true">ผ่าน</option>
                <option :value="false">ไม่ผ่าน</option>
                <option :value="''">รอผลสัมภาษณ์</option>
              </select>
              <select v-else-if="student.gotCompany == true" v-model="student.secondChoice.result"
                @change="onChangeSecondChoice($event,student.id)" class="selectData" disabled>
                <option :value="true">ผ่าน</option>
                <option :value="false">ไม่ผ่าน</option>
                <option :value="''">รอผลสัมภาษณ์</option>
              </select>
              <button v-if="student.secondChoice.result === 'true' && student.gotCompany !== true"
                class="btn btn-primary mt-2"
                @click="chooseCompany(student.secondChoice.project_Id,student.secondChoice.companyNameAndProjectName,student.id,student.userId)">เลือกบริษัท</button>
            </td>
            <td>
              <router-link class="text-decoration-none" style="color: black;"
                :to="{ path: `/CompanyDetails/${student.thirdChoice.project_Id}` }">
                {{ student.thirdChoice.companyNameAndProjectName }}</router-link>
              <select v-if="student.gotCompany !== true" v-model="student.thirdChoice.result"
                @change="onChangeThirdChoice($event,student.id)" class="selectData">
                <option :value="true">ผ่าน</option>
                <option :value="false">ไม่ผ่าน</option>
                <option :value="''">รอผลสัมภาษณ์</option>
              </select>
              <select v-else-if="student.gotCompany == true" v-model="student.thirdChoice.result"
                @change="onChangeThirdChoice($event,student.id)" class="selectData" disabled>
                <option :value="true">ผ่าน</option>
                <option :value="false">ไม่ผ่าน</option>
                <option :value="''">รอผลสัมภาษณ์</option>
              </select>
              <button v-if="student.thirdChoice.result === 'true' && student.gotCompany !== true"
                class="btn btn-primary  mt-2"
                @click="chooseCompany(student.thirdChoice.project_Id,student.thirdChoice.companyNameAndProjectName,student.id,student.userId)">เลือกบริษัท</button>
            </td>
            <td v-if="student.gotCompany !== true">
              <router-link :to="{ path: `/AdminEditStudent/${student.id}` }" class="text-decoration-none btn edit-btn"
                style="color:white;">
                แก้ไขบริษัท
              </router-link>
    ...
    methods: {
      ...
      async onChangeFirstChoice(event, studentDataId) {
        ...
        await updateDoc(studentRef, {
          'firstChoice.result': event.target.value
        });
        this.loading = true
        setTimeout(() => {
          this.loading = false;
      ...
      async onChangeSecondChoice(event, studentDataId) {
        ...
        await updateDoc(studentRef, {
          'secondChoice.result': event.target.value
        });
        this.loading = true
        setTimeout(() => {
          this.loading = false;
      ...
      async onChangeThirdChoice(event, studentDataId) {
        ...
        await updateDoc(studentRef, {
          'thirdChoice.result': event.target.value
        });
        this.loading = true
        setTimeout(() => {
          this.loading = false;
      ...
      async chooseCompany(companyDataId, companyName, studentDataId) {
        ...
        if (window.confirm("ต้องการเลือกบริษัทให้ : [ นาย " + studentData.firstName + " " + studentData.lastName +
            " ] ฝึกงานที่บริษัท : [ " + companyName + " ] ใช่หรือไม่?")) {
          await updateDoc(studentRef, {
          'companyConfirm': true
          });
      ...
      async confirmCompany(studentDataId) {
        ...
        let student = await getDoc(this.studentDoc);
        let studentData = student.data();
        if (window.confirm("คุณต้องการยืนยันบริษัทของนักศึกษาชื่อ : [ " + studentData.firstName + " " + studentData
            .lastName + " ] รหัสนักศึกษา : [ " + studentData.studentID + " ] ใช่หรือไม่?")) {
          await updateDoc(studentRef, {
            'companyConfirm': true
          });
