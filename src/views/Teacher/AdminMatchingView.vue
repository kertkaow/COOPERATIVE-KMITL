                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="Matching in Matchings" :key="Matching.id">
                                 <td>
                                   <router-link :to="{ path: `/MatchingDetails/${Matching.id}` }" class="text-decoration-none btn btn-primary" style="color:white;" id="button">
                                    ข้อมูลสหกิจ
                                    </router-link>
                                 </td>
                                 ...
                                 <td>
                                 <router-link :to="{ path: `/MemoDetails/${Matching.id}` }" class="text-decoration-none btn btn-primary" style="color:white;">
                                         ดู Memo
                                    </router-link></td>
                                 ...
                                 <td>
                                    <router-link :to="{ path: `/EditMatchingDetails/${Matching.id}` }" class="text-decoration-none btn edit-btn" style="color:white;">
                                         อัพเดต
                                    </router-link>
...
<script>
import { matchingCollection, studentCollection } from "@/firebase";
import { getDocs, doc, deleteDoc, query, updateDoc, orderBy} from "firebase/firestore";
...
export default {
    ...
    methods:{
      ...
      const orderByDirector = query(matchingCollection, orderBy("companyData.projectTeacherDirector.userName", "asc"))
      ...
      const orderByCompanyName = query(matchingCollection, orderBy("companyData.projectTeacherDirector.userName", "asc"))
      ...
      this.MatchingsProject = MatchingsProject; 
       ...
       if(window.confirm("ต้องการลบข้อมูลสหกิจของ : [ "+ firstName + " " +  lastName + "  (" + studentID +") ] " + " บริษัท : [ " + companyName + " ] " + "โครงการ : [ " + projectName + " ] " + "ใช่หรือไม่?")) {
       ...
       this.$router.push("/AdminMatching");
       }
      },
