...
<div class="sidebar">
  ...
  <a href="/active" style="background-color: #FF6E30;color:#FFFF">กรรมการนิเทศ</a>
        ...
        <div class="col-10">  
            ...
            <div class="d-flex head-container"><h1 class="mb-4 formtitle-1">ข้อมูลสหกิจที่เป็น [ กรรมการนิเทศ ]</h1></div>
            <div class="row">
                <div class="col-md-12">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="MatchingDirector in MatchingsDirector" :key="MatchingDirector.id">
                                 <td><router-link :to="{ path: `/MatchingDetails/${MatchingDirector.id}` }" class="text-decoration-none btn btn-primary" style="color:white;">
                                    ข้อมูลสหกิจ
                                    </router-link></td>
                                 ...
                                 <td> <router-link :to="{ path: `/DirectorWatchMemo/${MatchingDirector.id}` }" class="text-decoration-none btn btn-primary" style="color:white;">
                                         ดู Memo
                                    </router-link></td>
                                 ...
                                 <td>
                                    <router-link :to="{ path: `/EditMatchingDetails/${MatchingDirector.id}` }" class="text-decoration-none" style="color:white;">
...
import { getDocs,query, where,} from "firebase/firestore";
    ...
    methods: {
        ...
        const getMatchingDataByStudentUserId = query(matchingCollection, where("companyData.projectTeacherMentor.userId", "==", userId));
      ...
      async fetchMatchingDirectorByUserId() {
        const userId = sessionStorage.getItem("userId")
        const getMatchingDataByStudentUserId = query(matchingCollection, where("companyData.projectTeacherDirector.userId", "==", userId));
        const matchingSnapShot = await getDocs(getMatchingDataByStudentUserId);
